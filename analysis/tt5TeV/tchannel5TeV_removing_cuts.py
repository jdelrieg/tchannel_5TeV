...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        'counts_nlightjets'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts_nlightjets",  "Counts", 1, 0, 10)),
        'counts_nheavyjets'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts_nheavyjets",  "Counts", 1, 0, 10)),
...
######### hists for filling with th MVA cut
    ...
    def process(self, events):
        ...
        histAxisName_control = self._samples[dataset]["histAxisName"]   #Repeated for the case of Wjets
    ...
    #            cut=ak.flatten(cut)
     #           cut = np.array(cut, dtype=bool)
        ...
        #        cutnomet = (cutsel) & (cutjets) & (cutht)
         #       cutnomet=ak.flatten(cutnomet)
          #      cutnomet = np.array(cutnomet, dtype=bool)  
              ...
              #counts_njets=np.ones(len(ak.flatten(goodJets[cut])),dtype=float)
              #counts_nheavyjets=np.ones(sum(ak.flatten(goodJets.hadronFlavour[cut]>=4)),dtype=float)
      ...
      #### for splitting wjets                  ########                     
              ...
              wjets_flavour_dict={'cut_heavyjets':cut_heavyjets,'cut_lightjets':cut_lightjets,'cutnomet_heavyjets':cutnomet_heavyjets,'cutnomet_lightjets':cutnomet_lightjets,'heavyjets':histAxisName[:2]+'_heavyjets','lightjets':histAxisName[:2]+'_lightjets'}
      ...
      #         if cat!='dummy': cutname='cut_'+cat; cut=wjets_flavour_dict[cutname];cutnometname='cutnomet_'+cat; cutnomet=wjets_flavour_dict[cutnometname]; histAxisName=wjets_flavour_dict[cat];#weights=weights[cut]; 
            ...
            #  hout['mtwnocut'].fill(sample=histAxisName, channel=ch, level=lev, mtwnocut=mtw[cutnomet], syst=syst, weight=weights[cutnomet])
              ...
              weights_heavy=weights[cut_heavyjets]
              weights_light=weights[cut_lightjets]
              ...
              hout['counts_nlightjets'].fill(sample=histAxisName, channel=ch, level=lev, counts_nlightjets=counts[cut_lightjets], syst=syst, weight=weights_light)
              hout['counts_nheavyjets'].fill(sample=histAxisName, channel=ch, level=lev, counts_nheavyjets=counts[cut_heavyjets], syst=syst, weight=weights_heavy)
...
#                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                     if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                       ...
                       varsb6 = [ fmjj, fdrjjmed, abs(u0eta)[cut],invariant_masses[cut],mtw[cut],abs(b0eta-u0eta)[cut]]
                       

                       MVAscore_nom_2j1b = self.model_2j1b.predict_proba(np.column_stack(vars2j1b))[:,1] # asipara 2 clases
                       MVAscore_nom_bi = self.model_bi.predict_proba(np.column_stack(vars2j1b))[:,0]  #asi para 3 clases
                       MVAscore_nom_b6 = self.model_b6.predict_proba(np.column_stack(varsb6))[:,1] # asipara 2 clases
