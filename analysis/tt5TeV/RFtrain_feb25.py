...
vars_train=['2j1b_topmass','2j1b_mt','2j1b_mlb','2j1b_mjj','2j1b_mub','2j1b_sumAllPt','2j1b_ht_atlas','2j1b_DptWub','2j1b_DphiWub']
...
depth=9
split=14
leaf=1              #Values chosen based on halvingrandomsearchcv and manually changing a little to prevent from overfitting
                    #(values used previously in the ones that gives a little of overfitting: {'n_estimators': 400, 'min_samples_split': 7, 'min_samples_leaf': 4, 'max_depth': 6})
...
'''
df_train, df_test = BuildDataset(path, signal, bkg, vars_train, trainFrac, 21, nData=None)#,proportions=[0.167,0.166,0.167,0.166])# this is the number of Wjets nData=614)#
...
plt.savefig('mva/newstats/training//%s_ROCcurve_CMS.pdf'%name)
plt.savefig('mva/newstats/training//%s_ROCcurve_CMS.png'%name)
...
plt.grid(True)
plt.savefig('mva/newstats/training//%s_probs.pdf'%name)
plt.savefig('mva/newstats/training//%s_probs.png'%name)
plt.show()
...
plt.savefig('mva/newstats/training//%s_matrix.pdf'%name)
plt.savefig('mva/newstats/training//%s_matrix.png'%name)
...
import pickle
pickle.dump(model, open('mva/newstats/training//%s_p2v2.pkl'%name, 'wb'),protocol=2) 
pickle.dump(model, open('mva/newstats/training//%s.pkl'%name, 'wb')) 
