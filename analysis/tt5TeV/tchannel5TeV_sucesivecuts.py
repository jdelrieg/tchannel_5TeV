...
########## Hists I added
        'ht_atlas'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("ht_atlas",      "H'$_{T}$ (GeV)", 24, 5, 365)),
        ...
        'mtwnocut'   : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("mtwnocut",     "m$_{T}^{W}$ (GeV)", 15, 0, 150)),
        'ht_atlas_nocut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("ht_atlas_nocut",      "H'$_{T}$ (GeV)", 24, 5, 365)),
...
########################  corte sin na #####              
              ...
              cutnothing = (cutsel) & (cutjets) #& (cutmtw) & (cutht) #& (cutmlb) 

              cutnothing=ak.flatten(cutnothing)
              cutnothing = np.array(cutnothing, dtype=bool)												#main cut
              #print('channel',ch,'level',lev,'cut',sum(cut))
...
########################  corte con met #####              
              ...
              cutmet = (cutsel) & (cutjets) #& (cutmtw) & (cutht) #& (cutmlb) 

              cutmet=ak.flatten(cutmet)
              cutmet = np.array(cutmet, dtype=bool)	              
########################  corte con mtw #####              
              
              cuts      = [ch] + [lev] + ['metg30'] 
              cutjets = GetCutJets(cuts, syst, met_pt, njets_var, nbtags_var)  #met_pt en vez de mtw si queremos de verdad estimar qcd con met y no con mtw
              
              cut_mtw = (cutsel) & (cutjets) & (cutmtw) #& (cutht) #& (cutmlb) 

              cut_mtw=ak.flatten(cut_mtw)
              cut_mtw = np.array(cut_mtw, dtype=bool)	  
...
######################################  Redefine cuts from here if wanting to add something on the w-top system ##############################################################
              ...
              for cat in jetcategories:
               ...
               hout['metnocut'].fill(sample=histAxisName, channel=ch, level=lev, metnocut=met.pt[cutnothing], syst=syst, weight=weights[cutnothing])
               
               hout['mtwnocut'].fill(sample=histAxisName, channel=ch, level=lev, mtwnocut=mtw[cutmet], syst=syst, weight=weights[cutmet])
               hout['ht_atlas_nocut'].fill(sample=histAxisName, channel=ch, level=lev, ht_atlas_nocut=ht_atlas[cut_mtw], syst=syst, weight=weights[cut_mtw])
