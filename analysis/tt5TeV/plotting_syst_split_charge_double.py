...
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tchannel_hdampUp_part0.pkl.gz', 'rb') as file:
    loaded_hdampup = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tchannel_hdampDown_part0.pkl.gz', 'rb') as file:
    loaded_hdampdown = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tchannel_hdampUp_part1.pkl.gz', 'rb') as file:
    loaded_hdampup_bis = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tchannel_hdampDown_part1.pkl.gz', 'rb') as file:
    loaded_hdampdown_bis = pickle.load(file)
...
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tbarchannel_hdampUp_part0.pkl.gz', 'rb') as file:
    loaded_hdampup_bar = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tbarchannel_hdampDown_part0.pkl.gz', 'rb') as file:
    loaded_hdampdown_bar = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tbarchannel_hdampUp_part1.pkl.gz', 'rb') as file:
    loaded_hdampup_bar_bis = pickle.load(file)
with gzip.open('/mnt_pool/c3_users/user/jriego/tchannel5TeV/splitting_tchan_tbar/syst_check/tbarchannel_hdampDown_part1.pkl.gz', 'rb') as file:
    loaded_hdampdown_bar_bis = pickle.load(file)
...
if __name__ == '__main__':
	...
	parser.add_argument('--sample', type=str, default='top', help='Description of option')
...
#	h1error=np.array([sum(h1error[:4]),sum(h1error[4:6]),sum(h1error[6:8]),sum(h1error[8:10]),sum(h1error[10:12]),sum(h1error[12:14]),sum(h1error[14:16]),sum(h1error[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h1error[:])#np.array([sum(h1error[:5]),h1error[5],h1error[6],h1error[7],h1error[8],h1error[9],sum(h1error[10:])]) 
	...
	if sample == "top":
		...
		h1=np.array([sum(h1_bar[:4]),sum(h1_bar[4:6]),sum(h1_bar[6:8]),sum(h1_bar[8:10]),sum(h1_bar[10:12]),sum(h1_bar[12:14]),sum(h1_bar[14:16]),sum(h1_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h1_bar[:])#np.array([sum(h1[:5]),h1[5],h1[6],h1[7],h1[8],h1[9],sum(h1[10:])]) 
		h1error=np.array([sum(h1error_bar[:4]),sum(h1error_bar[4:6]),sum(h1error_bar[6:8]),sum(h1error_bar[8:10]),sum(h1error_bar[10:12]),sum(h1error_bar[12:14]),sum(h1error_bar[14:16]),sum(h1error_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h1error_bar[:])#np.array([sum(h1error[:5]),h1error[5],h1error[6],h1error[7],h1error[8],h1error[9],sum(h1error[10:])]) 
	else:	
...
#	h1=h1+h1_bar
	...
	h2=h2*302
	h2_bis,h2error_bis=loaded_hdampup_bis[var].values(sumw2=True)[('tchannel_hdampUp',ch,level,'norm')] if nuisance=='hdamp' else loaded_ueup[var].values(sumw2=True)[('tchannel_UEUp',ch,level,'norm')]
	h2_bis=h2_bis*302
	...
	h2_bar=h2_bar*302
	h2_bar_bis,h2error_bar_bis=loaded_hdampup_bar_bis[var].values(sumw2=True)[('tbarchannel_hdampUp',ch,level,'norm')] if nuisance=='hdamp' else loaded_ueup_bar[var].values(sumw2=True)[('tbarchannel_UEUp',ch,level,'norm')]
	h2_bar_bis=h2_bar_bis*302	
	...
	h3=h3*302
	h3_bis,h3error_bis=loaded_hdampdown_bis[var].values(sumw2=True)[('tchannel_hdampDown',ch,level,'norm')] if nuisance=='hdamp' else loaded_uedown[var].values(sumw2=True)[('tchannel_UEDown',ch,level,'norm')]	
	h3_bis=h3_bis*302
...
#	h3_bis=h3_bis*302
	...
	h3_bar=h3_bar*302
	h3_bar_bis,h3error_bar_bis=loaded_hdampdown_bar_bis[var].values(sumw2=True)[('tbarchannel_hdampDown',ch,level,'norm')] if nuisance=='hdamp' else loaded_uedown_bar[var].values(sumw2=True)[('tbarchannel_UEDown',ch,level,'norm')]	
	h3_bar_bis=h3_bar_bis*302
	
	#Up to here hdampdown
#	h2=h2+h2_bar
#	h3=h3+h3_bar
	if sample == "top":
		h2=np.array([sum(h2[:4]),sum(h2[4:6]),sum(h2[6:8]),sum(h2[8:10]),sum(h2[10:12]),sum(h2[12:14]),sum(h2[14:16]),sum(h2[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2[:])#np.array([sum(h2[:5]),h2[5],h2[6],h2[7],h2[8],h2[9],sum(h2[10:])]) 
		h2error=np.array([sum(h2error[:4]),sum(h2error[4:6]),sum(h2error[6:8]),sum(h2error[8:10]),sum(h2error[10:12]),sum(h2error[12:14]),sum(h2error[14:16]),sum(h2error[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2error[:])#np.array([sum(h2error[:5]),h2error[5],h2error[6],h2error[7],h2error[8],h2error[9],sum(h2error[10:])]) 
	elif sample == "tbar":
		h2=np.array([sum(h2_bar[:4]),sum(h2_bar[4:6]),sum(h2_bar[6:8]),sum(h2_bar[8:10]),sum(h2_bar[10:12]),sum(h2_bar[12:14]),sum(h2_bar[14:16]),sum(h2_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2_bar[:])#np.array([sum(h2[:5]),h2[5],h2[6],h2[7],h2[8],h2[9],sum(h2[10:])]) 
		h2error=np.array([sum(h2error_bar[:4]),sum(h2error_bar[4:6]),sum(h2error_bar[6:8]),sum(h2error_bar[8:10]),sum(h2error_bar[10:12]),sum(h2error_bar[12:14]),sum(h2error_bar[14:16]),sum(h2error_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2error_bar[:])#np.array([sum(h2error[:5]),h2error[5],h2error[6],h2error[7],h2error[8],h2error[9],sum(h2error[10:])]) 
	else:
		...
		h2=np.array([sum(h2[:4])+sum(h2_bar[:4])+sum(h2_bis[:4])+sum(h2_bar_bis[:4]),sum(h2[4:6])+sum(h2_bar[4:6])+sum(h2_bis[4:6])+sum(h2_bar_bis[4:6]),sum(h2[6:8])+sum(h2_bar[6:8])+sum(h2_bis[6:8])+sum(h2_bar_bis[6:8]),sum(h2[8:10])+sum(h2_bar[8:10])+sum(h2_bis[8:10])+sum(h2_bar_bis[8:10]),sum(h2[10:12])+sum(h2_bar[10:12])+sum(h2_bis[10:12])+sum(h2_bar_bis[10:12]),sum(h2[12:14])+sum(h2_bar[12:14])+sum(h2_bis[12:14])+sum(h2_bar_bis[12:14]),sum(h2[14:16])+sum(h2_bar[14:16])+sum(h2_bis[14:16])+sum(h2_bar_bis[14:16]),sum(h2[16:])+sum(h2_bar[16:])+sum(h2_bis[16:])+sum(h2_bar_bis[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2[:]+h2_bar[:])#np.array([sum(h2[:5]),h2[5],h2[6],h2[7],h2[8],h2[9],sum(h2[10:])]) 
		h2error=np.array([sum(h2error[:4])+sum(h2error_bar[:4])+sum(h2error_bis[:4])+sum(h2error_bar_bis[:4]),sum(h2error[4:6])+sum(h2error_bar[4:6])+sum(h2error_bis[4:6])+sum(h2error_bar_bis[4:6]),sum(h2error[6:8])+sum(h2error_bar[6:8])+sum(h2error_bis[6:8])+sum(h2error_bar_bis[6:8]),sum(h2error[8:10])+sum(h2error_bar[8:10])+sum(h2error_bis[8:10])+sum(h2error_bar_bis[8:10]),sum(h2error[10:12])+sum(h2error_bar[10:12])+sum(h2error_bis[10:12])+sum(h2error_bar_bis[10:12]),sum(h2error[12:14])+sum(h2error_bar[12:14])+sum(h2error_bis[12:14])+sum(h2error_bar_bis[12:14]),sum(h2error[14:16])+sum(h2error_bar[14:16])+sum(h2error_bis[14:16])+sum(h2error_bar_bis[14:16]),sum(h2error[16:])+sum(h2error_bar[16:])+sum(h2error_bis[16:])+sum(h2error_bar_bis[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h2error[:]+h2error_bar[:])#np.array([sum(h2error[:5]),h2error[5],h2error[6],h2error[7],h2error[8],h2error[9],sum(h2error[10:])]) 
	
	
	if sample == "top":
		h3=np.array([sum(h3[:4]),sum(h3[4:6]),sum(h3[6:8]),sum(h3[8:10]),sum(h3[10:12]),sum(h3[12:14]),sum(h3[14:16]),sum(h3[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3[:])#np.array([sum(h3[:5]),h3[5],h3[6],h3[7],h3[8],h3[9],sum(h3[10:])]) 
		h3error=np.array([sum(h3error[:4]),sum(h3error[4:6]),sum(h3error[6:8]),sum(h3error[8:10]),sum(h3error[10:12]),sum(h3error[12:14]),sum(h3error[14:16]),sum(h3error[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3error[:])#np.array([sum(h3error[:5]),h3error[5],h3error[6],h3error[7],h3error[8],h3error[9],sum(h3error[10:])]) 
	elif sample == "tbar":
		h3=np.array([sum(h3_bar[:4]),sum(h3_bar[4:6]),sum(h3_bar[6:8]),sum(h3_bar[8:10]),sum(h3_bar[10:12]),sum(h3_bar[12:14]),sum(h3_bar[14:16]),sum(h3_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3_bar[:])#np.array([sum(h3[:5]),h3[5],h3[6],h3[7],h3[8],h3[9],sum(h3[10:])]) 
		h3error=np.array([sum(h3error_bar[:4]),sum(h3error_bar[4:6]),sum(h3error_bar[6:8]),sum(h3error_bar[8:10]),sum(h3error_bar[10:12]),sum(h3error_bar[12:14]),sum(h3error_bar[14:16]),sum(h3error_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3error_bar[:])#np.array([sum(h3error[:5]),h3error[5],h3error[6],h3error[7],h3error[8],h3error[9],sum(h3error[10:])]) 
	else:
		...
		h3=np.array([sum(h3[:4])+sum(h3_bar[:4])+sum(h3_bis[:4])+sum(h3_bar_bis[:4]),sum(h3[4:6])+sum(h3_bar[4:6])+sum(h3_bis[4:6])+sum(h3_bar_bis[4:6]),sum(h3[6:8])+sum(h3_bar[6:8])+sum(h3_bis[6:8])+sum(h3_bar_bis[6:8]),sum(h3[8:10])+sum(h3_bar[8:10])+sum(h3_bis[8:10])+sum(h3_bar_bis[8:10]),sum(h3[10:12])+sum(h3_bar[10:12])+sum(h3_bis[10:12])+sum(h3_bar_bis[10:12]),sum(h3[12:14])+sum(h3_bar[12:14])+sum(h3_bis[12:14])+sum(h3_bar_bis[12:14]),sum(h3[14:16])+sum(h3_bar[14:16])+sum(h3_bis[14:16])+sum(h3_bar_bis[14:16]),sum(h3[16:])+sum(h3_bar[16:])+sum(h3_bis[16:])+sum(h3_bar_bis[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3[:]+h3_bar[:])#np.array([sum(h3[:5]),h3[5],h3[6],h3[7],h3[8],h3[9],sum(h3[10:])]) 
		h3error=np.array([sum(h3error[:4])+sum(h3error_bar[:4])+sum(h3error_bis[:4])+sum(h3error_bar_bis[:4]),sum(h3error[4:6])+sum(h3error_bar[4:6])+sum(h3error_bis[4:6])+sum(h3error_bar_bis[4:6]),sum(h3error[6:8])+sum(h3error_bar[6:8])+sum(h3error_bis[6:8])+sum(h3error_bar_bis[6:8]),sum(h3error[8:10])+sum(h3error_bar[8:10])+sum(h3error_bis[8:10])+sum(h3error_bar_bis[8:10]),sum(h3error[10:12])+sum(h3error_bar[10:12])+sum(h3error_bis[10:12])+sum(h3error_bar_bis[10:12]),sum(h3error[12:14])+sum(h3error_bar[12:14])+sum(h3error_bis[12:14])+sum(h3error_bar_bis[12:14]),sum(h3error[14:16])+sum(h3error_bar[14:16])+sum(h3error_bis[14:16])+sum(h3error_bar_bis[14:16]),sum(h3error[16:])+sum(h3error_bar[16:])+sum(h3error_bis[16:])+sum(h3error_bar_bis[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3error[:]+h3error_bar[:])#np.array([sum(h3error[:5]),h3error[5],h3error[6],h3error[7],h3error[8],h3error[9],sum(h3error[10:])]) 
...
#	h3error=np.array([sum(h3error[:4])+sum(h3error_bar[:4]),sum(h3error[4:6])+sum(h3error_bar[4:6]),sum(h3error[6:8])+sum(h3error_bar[6:8]),sum(h3error[8:10])+sum(h3error_bar[8:10]),sum(h3error[10:12])+sum(h3error_bar[10:12]),sum(h3error[12:14])+sum(h3error_bar[12:14]),sum(h3error[14:16])+sum(h3error_bar[14:16]),sum(h3error[16:])+sum(h3error_bar[16:])]) if var=='MVAscore_relaxed_b10' else np.array(h3error[:]+h3error_bar[:])#np.array([sum(h3error[:5]),h3error[5],h3error[6],h3error[7],h3error[8],h3error[9],sum(h3error[10:])]) 
	...
	plt.savefig("lowess/doubling/{}_{}_{}{}{}.png".format(nuisance,'no_lowess' if doLowess==False else 'lowess', ch+level,'_uncs' if uncs else None,'_'+sample))
	exit()
