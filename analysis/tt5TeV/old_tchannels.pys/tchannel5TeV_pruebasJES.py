...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        'absu0eta_nocut'      : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("absu0eta_nocut", "Leading u-jet $|\eta|$ ", 10, 0, 5)),        
        'jetpt'       : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("jetpt",  "Jet $p_{T}$ (GeV)", 10, 0, 300)),
        ...
        'MVAscore_2p4'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("MVAscore_2p4", "MVA score", 20,0,1 )),
...
######### hists for filling with th MVA cut
        ...
        '2j1b_jeteta_nocut' : processor.column_accumulator(np.array([])),
        '2j1b_jetpt_nocut' : processor.column_accumulator(np.array([])),
        '2j1b_njets_nocut' : processor.column_accumulator(np.array([])),
        
        '2j1b_u0eta_nocut' : processor.column_accumulator(np.array([])),
        '2j1b_u0pt_nocut' : processor.column_accumulator(np.array([])),
    ...
    def process(self, events):
        ...
        if not isData:
          ...
          #print('before JEc fields',corrected_jets.fields)
          #print('before JEC pt',*corrected_jets.pt)
          #print('jet eta',*corrected_jets.eta)
          corte_eta=abs(corrected_jets.eta)>3.9
          
          corte=corte_eta #& corte_pt
          #print('before JEC pt',*corrected_jets.pt[corte])
          #print()
          #print('corte eta',*corte_eta,'\n corte pt',*corte_pt,'\n corte', *corte)
          #print('corte eta',corte_eta)
          corrected_jets = jet_factory.build(cleanedJets, lazy_cache=events_cache)
          corte_pt=corrected_jets.pt_raw>30
          
          #print('after JEC pt orig',*ak.flatten(corrected_jets.pt_raw[corte]))
          #print('after JEC eta',*corrected_jets.eta)
          #print('after JEc fields',corrected_jets.fields)
          print('after JEC  cut',*corrected_jets.pt)
          #print('after JEC eta cut',*ak.flatten(corrected_jets.pt[corte]))
          
          
          cleanedJets = corrected_jets
          #print('fields cleaned',cleanedJets.fields)
        ...
        goodJets, metpt, btagSF = GetJESMETvar(corrected_jets, met, jetptcut, btagwp=wp, isData=isData,var='')
        goodJets_trick, metpt_trick, btagSF_trick = GetJESMETvar(corrected_jets[corte_pt], met, jetptcut=0, btagwp=wp, isData=isData, var='')
        #print('nominal',len(ak.flatten(goodJets)),'\n los que entran',len(ak.flatten(jets[corte_pt])))
       ...
       # selections.add('lep_plus',((events.isl_plus)&((trige)|(trigm))))
        ...
        njets_nom, nbtags_nom, nujets_nom = GetNjetNbtagsNujets(goodJets_trick)
        ...
        #u0_nom, druu_nom, druumedian_nom, muu_nom, ptuu_nom = GetJetVariables(goodJets_trick[(goodJets_trick.isBtag==0)])
        ...
        for syst in systList:
          ...
          if syst in systJets:
            ...
            goodJets_jes, metpt_jes, btagSF_jes = GetJESMETvar(corrected_jets[corte_pt], met, jetptcut=0, btagwp=wp, isData=isData, var=syst)
            
            #print('met',*met.pt)
            njets, nbtags, nujets = GetNjetNbtagsNujets(goodJets)
            njets_var = njets

            njets_jes, nbtags_jes, nujets_jes = GetNjetNbtagsNujets(goodJets_jes)
            njets_var_jes = njets_jes
...
#              cutnomet=cutnomet&cuttop  #& cut0bs
             ...
             #  hout['mtwnocut'].fill(sample=histAxisName, channel=ch, level=lev, mtwnocut=mtw[cutnomet], syst=syst, weight=weights[cutnomet])
               ...
               #corte_pt=goodJets.pt>25
               #print('channel',ch,'syst',syst,'jetpt',*(goodJets.pt)[corte_pt],'\n amount jets',len(ak.flatten(goodJets.pt)[ak.flatten(corte_pt)]))
               #print('channel',ch,'syst',syst,'njets',*njets_var)
          #     hout[f'{lev}_jeteta_nocut'] = processor.column_accumulator(ak.flatten(goodJets.eta).to_numpy())
          #     njets_repeated=np.repeat(njets_var, njets_var)
          #     hout[f'{lev}_njets_nocut'] = processor.column_accumulator(njets_repeated.to_numpy())
               cuts      = [lev] + ['metg30'] 
               cutjets = GetCutJets(cuts, syst, met_pt, njets_var, nbtags_var)  #met_pt en vez de mtw si queremos de verdad estimar qcd con met y no con mtw
               ...
               #cut=ak.flatten(cut)
               cut = np.array(cut, dtype=bool)	        
               if syst in systJets: hout[f'{lev}_jetpt_nocut{syst}'] = processor.column_accumulator(ak.flatten(goodJets_jes.pt).to_numpy())
               if syst in ["norm"]: hout[f'{lev}_jetpt_nocut{syst}'] = processor.column_accumulator(ak.flatten(goodJets_trick.pt).to_numpy());hout[f'{lev}_jeteta_nocut'] = processor.column_accumulator(ak.flatten(goodJets_trick.eta).to_numpy());njets_repeated=np.repeat(njets_var, njets_var);hout[f'{lev}_njets_nocut'] = processor.column_accumulator(njets_repeated.to_numpy())
               if syst in ["norm"]: hout[f'{lev}_u0pt_nocut{syst}'] = processor.column_accumulator(u0pt[cut].to_numpy());hout[f'{lev}_u0eta_nocut'] = processor.column_accumulator(u0eta[cut].to_numpy());hout[f'{lev}_u0njets_nocut'] = processor.column_accumulator(njets_var[cut].to_numpy())#;njets_repeated=np.repeat(njets_var, njets_var);

               #print('syst',syst,'weights',*weights)
               hout['absu0eta_nocut'].fill(sample=histAxisName, channel=ch, level=lev, absu0eta_nocut=ak.fill_none(abs(u0eta),0), syst=syst, weight=weights) 
               hout['jetpt'].fill(sample=histAxisName, channel=ch, level=lev, jetpt=ak.fill_none(u0pt,0), syst=syst, weight=weights)    
...
#               if lev in ['2j1b','3j1b','3j2b']:hout['topeta'].fill(sample=histAxisName, channel=ch, level=lev, topeta=top_eta[cut], syst=syst, weight=weights)        
 ...
 #                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                      if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                        ...
                        vars2p4 = [abs(u0eta)[cut],invariant_masses[cut],abs(b0eta-u0eta)[cut],mt,mlb[cut],fdrjjmed,fmjj,mub[cut],DphiWub[cut],DptWub[cut]]
                        ...
                        MVAscore_nom_2p4 = self.model_eta2p4.predict_proba(np.column_stack(vars2p4))[:,1]
                        

                        cutMVA=MVAscore_nom_pruned>0.6
                        ...
                        hout['MVAscore_2p4'].fill(sample=histAxisName, channel=ch, level=lev, MVAscore_2p4=MVAscore_nom_2p4, syst=syst, weight=weights)
