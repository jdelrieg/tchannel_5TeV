...
########## Hists I added
        'ht_atlas'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("ht_atlas",      "H$_{T}$ ATLAS (GeV)", 6, 180, 360)),
...
########## hists i added end
    ...
    def process(self, events):
        ...
        # Muon selection
        mu["isLoose"] = MuonLoose(mu.pt, mu.eta, mu.dxy, mu.dz, mu.sip3d, mu.mediumPromptId, mu.btagDeepB, ptCut=20, etaCut=2.4)  #20,2.4
        ...
        e['isLoose'] = ElecLoose(e.pt, e.eta, e.lostHits, e.sip3d, e.dxy, e.dz, e.btagDeepB, e.convVeto, e.mvaFall17V2noIso_WPL, 20, 2.4) #20, 2.4
...
#              cutsnoMET = [ch] + [lev] + ['METfilters']
 #             cutjetsnomet = GetCutJets(cutsnoMET, syst, met_pt, njets_var, nbtags_var, nujets_var)
  #            cutnomet = (cutsel) & (cutjetsnomet) & (cutmlb) & (cutht) & (cutmtw) 
   #           cutnomet=ak.flatten(cutnomet)
    #          cutnomet = np.array(cutnomet, dtype=bool)										#special cuts for met validations   OJO QUE SE LLAMA MET por consistencia con el resto del codigo pero es mtw
              ...
              if ch in ['e','e_fake']:
                ...
                cut = (cutsel) & (cutjets) & (cutmlb) & (cutht)
                cut=ak.flatten(cut)
                ...
                cutsnoMET = [ch] + [lev] + ['METfilters']
                cutjetsnomet = GetCutJets(cutsnoMET, syst, met_pt, njets_var, nbtags_var, nujets_var)  
                cutnomet = (cutsel) & (cutjetsnomet) & (cutmlb) & (cutht)
                cutnomet=ak.flatten(cutnomet)
                cutnomet = np.array(cutnomet, dtype=bool)  

              else:             
                cuts      = [ch] + [lev] + ['METfilters']
                cutjets = GetCutJets(cuts, syst, met_pt, njets_var, nbtags_var)
                cut = (cutsel) & (cutjets) & (cutmtw) & (cutmlb) & (cutht)
                cut=ak.flatten(cut)
                cut = np.array(cut, dtype=bool)
                ...
                #cutsnoMET = [ch] + [lev] + ['METfilters']
                #cutjetsnomet = GetCutJets(cutsnoMET, syst, met_pt, njets_var, nbtags_var, nujets_var)  
                cutnomet = (cutsel) & (cutjets) & (cutmlb) & (cutht)
                cutnomet=ak.flatten(cutnomet)
                cutnomet = np.array(cutnomet, dtype=bool)  
...
######################################  Redefine cuts from here if wanting to add something on the w-top system ##############################################################
              ...
              cuttop=(invariant_masses>130)&(invariant_masses<230)
        ...
        #        cutnomet = (cutsel) & (cutjets) & (cutht)
         #       cutnomet=ak.flatten(cutnomet)
          #      cutnomet = np.array(cutnomet, dtype=bool)  
              ...
              if ch in['e','e_fake']:cut_metl25 = (cutnomet) & GetCutJets(['metl25'], syst, met_pt, njets_var, nbtags_var)
              else: cut_metl25 = (cutnomet) & GetCutJets(['metl45'], syst, mtw, njets_var, nbtags_var)
