...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        if isinstance(model, list) and len(model) >= 2:
          self.model_allvars = model[0]
          self.model_varspruned = model[1]
          self.model_b7 = model[2]
        ...
        'MVAscore_all'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("MVAscore_all", "MVA score", 20,0,1 )), #20,0,1  
        ...
        'MVAscore_b7'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("MVAscore_b7", "MVA score", 20,0,1 )),
...
#              cutnomet=cutnomet&cuttop  #& cut0bs
 ...
 #                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                      if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                        ...
                        varsall = [fht,abs(u0eta)[cut],fst,fptSumVecAll,leta,fj0pt,fj0eta,u0pt[cut],u0eta[cut],ak.flatten(ptjj[cut]),fmjj,fdrjjmed,fdrjjmin,mlb[cut],mt,ptSumVeclb[cut],fdRlb,ht_atlas[cut],mtw[cut],b0eta[cut],abs(b0eta-u0eta)[cut],invariant_masses[cut],u0mass[cut],abs(ak.flatten(lep_eta))[cut],top_eta[cut],abs(w_etas)[cut],dRub[cut],cos2[cut]]
                        varspruned = [fht,abs(u0eta)[cut],fptSumVecAll,leta,fj0pt,fj0eta,u0pt[cut],ak.flatten(ptjj[cut]),fmjj,fdrjjmed,mlb[cut],ptSumVeclb[cut],fdRlb,ht_atlas[cut],mtw[cut],b0eta[cut],abs(b0eta-u0eta)[cut],invariant_masses[cut],u0mass[cut],abs(ak.flatten(lep_eta))[cut],top_eta[cut],abs(w_etas)[cut],dRub[cut],cos2[cut]]
                        varsb7 = [abs(u0eta)[cut],abs(b0eta-u0eta)[cut],mtw[cut],fdrjjmed,invariant_masses[cut],fmjj,mlb[cut]]
                        

                        MVAscore_nom_all = self.model_allvars.predict_proba(np.column_stack(varsall))[:,1] # asipara 2 clases
                        #MVAscore_nom_bi = self.model_bi.predict_proba(np.column_stack(vars2j1b))[:,0]  #asi para 3 clases
                        MVAscore_nom_pruned = self.model_varspruned.predict_proba(np.column_stack(varspruned))[:,1] # asipara 2 clases
                        MVAscore_nom_b7 = self.model_b7.predict_proba(np.column_stack(varsb7))[:,1]


                        cutMVA=MVAscore_nom_all>0.6
                        ...
                        hout['MVAscore_b7'].fill(sample=histAxisName, channel=ch, level=lev, MVAscore_b7=MVAscore_nom_b7, syst=syst, weight=weights)
