...
#              cutbetal=abs(b0eta)<2.5
 #             cutbeta=ak.Array(cutbetal)
  #            cutbeta=ak.fill_none(cutbeta,False)    #For adding the eta cut in b jet (me)
   #           
              maskcutuetal = (1.5 < np.abs(u0eta)) & (np.abs(u0eta) < 4.7)
              cutueta=ak.fill_none(maskcutuetal,False)    #For adding the eta cut in non-b jet (me)
              
              maskcutuetal_noniso = (1.5 < np.abs(u0eta)) & (np.abs(u0eta) < 4.7)
              cutueta_noniso=ak.fill_none(maskcutuetal_noniso,False)    #For adding the eta cut in non-b jet (me)
              
              
              cutdeltaetal=abs(b0eta-u0eta)>1.0
...
########################################################################################################                   Up to here the cut thing
              ...
              cut = (cutsel) & (cutjets) & (cutmtw) & (cutht) & (cutmlb) & (cutueta) & (cutdeltaeta) 
              cut=ak.flatten(cut)
              cut = np.array(cut, dtype=bool)												#main cut
              #print('when defined cut',sum(cut))
              
              cut_noniso=(cutsel) & (cutjets) & (cutmtw) & (cutht) & (cutmlb) #& (cutueta_noniso)
              cut_noniso=ak.flatten(cut_noniso)
              cut_noniso = np.array(cut_noniso, dtype=bool)	
              #print('when defined cut_noniso',sum(cut_noniso))
              
              cutsnoMET = [ch] + [lev] 
              cutjetsnomet = GetCutJets(cutsnoMET, syst, met_pt, njets_var, nbtags_var, nujets_var)
              cutnomet = (cutsel) & (cutjetsnomet)   & (cutht) & (cutmlb) & (cutueta) & (cutdeltaeta) #& (cutmtw)
              ...
              cutnomet_noniso = (cutsel) & (cutjetsnomet)  & (cutht) & (cutmlb) #& (cutueta_noniso)  #& (cutmtw) 
              cutnomet_noniso=ak.flatten(cutnomet_noniso)
              cutnomet_noniso = np.array(cutnomet_noniso, dtype=bool)										#special cuts for met validations   OJO QUE SE LLAMA MET por consistencia con el resto del codigo pero es mtw              
...
######################################  Redefine cuts from here if wanting to add something on the w-top system ##############################################################
              ...
              cut_noniso=cut_noniso&cuttop    #& cut0bs
              cutnomet_noniso=cutnomet_noniso&cuttop  #& cut0bs
              ...
              #print('level',lev)
              #print('Antes','ch',ch,'cut',sum(cut))
              if ch in ['e_fake','m_fake']:cut=cut_noniso;cutnomet=cutnomet_noniso
              #print('Despues','ch',ch,'cut',sum(cut),'cut non iso',sum(cut_noniso))
              #print()
