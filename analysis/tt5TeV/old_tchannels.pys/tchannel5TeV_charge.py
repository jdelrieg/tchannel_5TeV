...
def GetCutJets(cuts, syst, metpt, njets, nbtags, nujets=None):
  ...
  elif 'metg180' in cuts:
    metcat = (metpt > 180)
  elif 'metg175' in cuts:
    metcat = (metpt > 175)
  elif 'metg170' in cuts:
    metcat = (metpt > 170)
  elif 'metl180' in cuts:
    metcat = (metpt < 180)
  elif 'metl175' in cuts:
    metcat = (metpt < 175)
  elif 'metl170' in cuts:
    metcat = (metpt < 170)
...
class AnalysisProcessor(processor.ProcessorABC):
    def __init__(self, samples, model):
        ...
        'counts_metg180': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
        'counts_metl180': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
        ...
        'counts_metg170': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
        'counts_metl170': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
        'counts_metg175': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
        'counts_metl175': hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("counts",  "Counts", 1, 0, 10)),
...
########## hists i added end
    ...
    def process(self, events):
      ...
      ##################################################################################################################  this defined in order to deal with charge instead of flavour  
        ...
        l_sel_plus = ak.with_name(ak.concatenate([e_sel_plus, m_sel_plus], axis=1), 'PtEtaPhiMCandidate')        
        l_sel_plus_padded = ak.pad_none(l_sel_plus, 1)
        lep_pluss0 = l_sel_plus_padded[:,0]        
        
        m_sel_minus=mu[mu.isLoose & mu.isMVA & (m_pdgId_new<0)]
        e_sel_minus=e[e.isLoose & e.isMVA & (e_pdgId_new<0)]
        
        l_sel_minus = ak.with_name(ak.concatenate([e_sel_minus, m_sel_minus], axis=1), 'PtEtaPhiMCandidate')        
        l_sel_minus_padded = ak.pad_none(l_sel_minus, 1)
        lep_minus0 = l_sel_minus_padded[:,0]           


        m_fake_plus=mu[mu.isLoose & (mu.isMVA==0) & (m_pdgId_new>0)]
        e_fake_plus=e[e.isLoose & (e.isMVA==0) & (e_pdgId_new>0)]
        
        l_fake_plus = ak.with_name(ak.concatenate([e_fake_plus, m_fake_plus], axis=1), 'PtEtaPhiMCandidate')        
        l_fake_plus_padded = ak.pad_none(l_fake_plus, 1)
        lep_pluss_fake0 = l_fake_plus_padded[:,0]   
        
        m_fake_minus=mu[mu.isLoose & (mu.isMVA==0) & (m_pdgId_new<0)]
        e_fake_minus=e[e.isLoose & (e.isMVA==0) & (e_pdgId_new<0)]
        
        l_fake_minus = ak.with_name(ak.concatenate([e_fake_minus, m_fake_minus], axis=1), 'PtEtaPhiMCandidate')        
        l_fake_minus_padded = ak.pad_none(l_fake_minus, 1)
        lep_minus_fake0 = l_fake_minus_padded[:,0]                 



        lp0=l_sel_plus[ak.argmax(l_sel_plus.pt,axis=-1,keepdims=True)]
        lm0=l_sel_minus[ak.argmax(l_sel_minus.pt,axis=-1,keepdims=True)]
        lpf0=l_fake_plus[ak.argmax(l_fake_plus.pt,axis=-1,keepdims=True)]
        lmf0=l_fake_minus[ak.argmax(l_fake_minus.pt,axis=-1,keepdims=True)]        
 ################################################################################################################################# Up to here (and some smaller changes in the rest of the code)       
        ...
        events['isl_plus']=((ak.num(m_sel) == 0) & (ak.num(e_sel) == 1)&(e0_pdgId_new>0))|((ak.num(m_sel) == 1) & (ak.num(e_sel) == 0)&(m0_pdgId_new>0))
        events['isl_minus']=((ak.num(m_sel) == 0) & (ak.num(e_sel) == 1)&(e0_pdgId_new<0))|((ak.num(m_sel) == 1) & (ak.num(e_sel) == 0)&(m0_pdgId_new<0))
        
        
        
        events['isl_plus_fake']=((ak.num(m_sel) == 0) & (ak.num(e_sel) == 0)&(ak.num(e_fake)==1)&(e_fake0_pdgId_new>0))|((ak.num(m_sel) ==0) & (ak.num(e_sel) == 0)&(ak.num(m_fake)==1)&(m_fake0_pdgId_new>0))
        events['isl_minus_fake']=((ak.num(m_sel) == 0) & (ak.num(e_sel) == 0)&(ak.num(e_fake)==1)&(e_fake0_pdgId_new<0))|((ak.num(m_sel) ==0) & (ak.num(e_sel) == 0)&(ak.num(m_fake)==1)&(m_fake0_pdgId_new<0))
        ...
        for ch_name in ["em", "e", "m", 'ee', 'mm','lep_pluss','lep_minus']:
        ...
        selections.add('lep_pluss',((events.isl_plus)&((trige)|(trigm))))
        selections.add('lep_minus',((events.isl_minus)&((trige)|(trigm))))

        selections.add('lep_pluss_fake',((events.isl_plus_fake)&((trige)|(trigm))))
        selections.add('lep_minus_fake',((events.isl_minus_fake)&((trige)|(trigm))))
        ...
        channels=['lep_minus','lep_pluss','lep_pluss_fake','lep_minus_fake']
        ...
        for syst in systList:
          ...
          for ch in channels:
            ...
            if ch in ['lep_pluss_fake', 'lep_minus_fake']:
...
#######################################################################################################    Adding by myself extra cuts to replicate ATLAS selection
              ...
              if ch == 'lep_pluss': mlb=GetMlb1b(lp0,b0)
              if ch== 'lep_minus': mlb=GetMlb1b(lm0,b0)
              if ch == 'lep_pluss_fake': mlb=GetMlb1b(lpf0,b0)
              if ch== 'lep_minus_fake': mlb=GetMlb1b(lmf0,b0)
...
########################################################################################################                   Up to here the cut thing
          ...
          #    cut=ak.flatten(cut)
              ...
              cutnomet = (cutsel) & (cutjets) & (cutbeta) & (cutueta) & (cutdeltaeta)  & (cutmtw) & (cutmlb) #   & (cutmet_mtw)
              ...
              weights = weights_dict[ch if not 'fake' in ch else ch[:9]].weight(syst if not syst in (['norm']+systJets) else None)
              
              
              #
       #       flat_ht_atlas = ak.to_numpy(ak.flatten(ht_atlas[cut]))
       #       flat_ht_atlas_nocut = ak.to_numpy(ak.flatten(ht_atlas[cut_noht]))
              flat_ht_atlas = ak.to_numpy(ht_atlas[cut])
              flat_ht_atlas_nocut = ak.to_numpy(ht_atlas[cutnomet])
              
              #flat_mtw = ak.to_numpy(ak.flatten(mtw[cut]))
              #print('channel',ch,'\n mlb',*mlb[cut1],'\n corte mlb',*cut[cut1])
              #print('fake pt',fake_pt[cutsel],'\n cutsel',cutsel)
              #print('lep_pt',lep_pt)
              #print('pdgId e',e_sel.pdgId[cut], '\n pdgId m', m_sel.pdgId[cut])
              #print('channel',ch,'\n nelec:',ak.num(e_sel)[cut],'\n n muon:',ak.num(m_sel)[cut],'\n lep_pt:',lep_pt[cut],'\n e_pt',*e_sel.pt[cut],'\n m_pt',*m_sel.pt[cut])#,'\n fake_pt:',fake_pt[cutsel],'\n mtw_fake',mtw_fake[cutsel],'\n corte',cut[cutsel],'\n corte en fakes',cut[cutsel])
...
#              cut_metg25 = (cutnomet) & GetCutJets(['metg25'], syst, mtw, njets_var, nbtags_var)   # de aqui   para looser 2 tengo 25 20 15
...
#              cut_metl30 = (cutnomet) & GetCutJets(['metl30'], syst, mtw, njets_var, nbtags_var)
#              cut_metg30 = (cutnomet) & GetCutJets(['metg30'], syst, mtw, njets_var, nbtags_var)
...
#              hout['ht_atlas_nocut'].fill(sample=histAxisName, channel=ch, level=lev, ht_atlas_nocut=flat_ht_atlas_nocut, syst=syst, weight=weights[cut_noht])
              ...
              # Fill met norm histos
              cut_metg180 = (cutnomet) & GetCutJets(['metg180'], syst, ht_atlas, njets_var, nbtags_var)   # de aqui
              cut_metl180 = (cutnomet) & GetCutJets(['metl180'], syst, ht_atlas, njets_var, nbtags_var)
              cut_metg175 = (cutnomet) & GetCutJets(['metg175'], syst, ht_atlas, njets_var, nbtags_var)
              cut_metl175 = (cutnomet) & GetCutJets(['metl175'], syst, ht_atlas, njets_var, nbtags_var)
              cut_metg15 = (cutnomet) & GetCutJets(['metg15'], syst, met_pt, njets_var, nbtags_var)
              cut_metg170 = (cutnomet) & GetCutJets(['metg170'], syst, ht_atlas, njets_var, nbtags_var) 
              cut_metl170 = (cutnomet) & GetCutJets(['metl170'], syst, ht_atlas, njets_var, nbtags_var)   # a aqui  mtw-> met_pt si queremos hacer el calculo de qcd con met y no mtw
              
             
              cut_metg180 = ak.Array([False if x is None else x for x in cut_metg180])
              cut_metl180 = ak.Array([False if x is None else x for x in cut_metl180])
              cut_metg175 = ak.Array([False if x is None else x for x in cut_metg175])
              cut_metl175 = ak.Array([False if x is None else x for x in cut_metl175])
              cut_metg170 = ak.Array([False if x is None else x for x in cut_metg170])
              cut_metl170 = ak.Array([False if x is None else x for x in cut_metl170])
              
              weights_metg180 = weights[cut_metg180]; weights_metl180 = weights[cut_metl180]
              weights_metg175 = weights[cut_metg175]; weights_metl175 = weights[cut_metl175]
              weights_metg170 = weights[cut_metg170]; weights_metl170 = weights[cut_metl170]
              weights_metg15 = weights[cut_metg15]
              hout['counts_metg180'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metg180], syst=syst, weight=weights_metg180)
              hout['counts_metl180'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metl180], syst=syst, weight=weights_metl180)
              hout['counts_metg175'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metg175], syst=syst, weight=weights_metg175)
              hout['counts_metg15'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metg15], syst=syst, weight=weights_metg15)
              hout['counts_metl175'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metl175], syst=syst, weight=weights_metl175)
              hout['counts_metg170'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metg170], syst=syst, weight=weights_metg170)
              hout['counts_metl170'].fill(sample=histAxisName, channel=ch, level=lev, counts=counts[cut_metl170], syst=syst, weight=weights_metl170)

              ### met dist with no cut
              #hout['metnocut'].fill(sample=histAxisName, channel=ch, level=lev, metnocut=met.pt[cutnomet], syst=syst, weight=weights[cutnomet])
              #hout['mtwnocut'].fill(sample=histAxisName, channel=ch, level=lev, mtwnocut=mtw[cutnomet], syst=syst, weight=weights[cutnomet])
              hout['ht_atlas_nocut'].fill(sample=histAxisName, channel=ch, level=lev, ht_atlas_nocut=ht_atlas[cutnomet], syst=syst, weight=weights[cutnomet])
