...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        if isinstance(model, list) and len(model) >= 2:
          ...
          self.model_relaxed_b10_fix = model[2]
        ...
        'MVAscore_relaxed_b10_fix'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("MVAscore_relaxed_b10_fix", "MVA score", 20,0,1 )),
 ...
 ###
               ...
               if fillAll:
                 ...
                 if ch in ['e_plus','e_minus', 'e_fake_plus','e_fake_minus']:
                   ...
                   if len(lpt)!=sum(cut):
                      if ch == 'e_fake_minus': recorte=(e_pdgId_new[cut])>0;print(ch,'antes',len(weights),type(weights));weights_copy=np.copy(weights);weights=weights[recorte];print('despues',len(weights));#lpt=lpt[recorte];mt=mt[recorte];leta=leta[recorte];
                      if ch == 'e_fake_plus': recorte=(e_pdgId_new[cut])<0;print(ch,'antes',len(weights),type(weights));weights_copy=np.copy(weights);weights=weights[recorte];print('despues',len(weights));#lpt=lpt[recorte];mt=mt[recorte];leta=leta[recorte]                   
...
#                   if lev in ['2j1b','3j1b','3j2b']:hout['coste_3'].fill(sample=histAxisName, channel=ch, level=lev, coste_3=cos2_abs[cut], syst=syst, weight=weights)
 ...
 #                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                      if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                        ...
                        if ak.count(variables)>0:MVAscore_nom_relaxed_b10 = self.model_relaxed_b10.predict_proba(variables)[:,1]; cutMVA=MVAscore_nom_relaxed_b10>0.5
                        if ak.count(variables)>0:MVAscore_nom_relaxed_b10_fix = self.model_relaxed_b10_fix.predict_proba(variables)[:,1]; cutMVA=MVAscore_nom_relaxed_b10_fix>0.5
                        ...
                        hout['MVAscore_relaxed_b10_fix'].fill(sample=histAxisName, channel=ch, level=lev, MVAscore_relaxed_b10_fix=MVAscore_nom_relaxed_b10_fix, syst=syst, weight=weights) if len(MVAscore_nom_relaxed_b10_fix)==len(weights) else hout['MVAscore_relaxed_b10_fix'].fill(sample=histAxisName, channel=ch, level=lev, MVAscore_relaxed_b10_fix=MVAscore_nom_relaxed_b10_fix, syst=syst, weight=weights_copy)
      
                        
                        hout['MVAscore_relaxed_b10_cut'].fill(sample=histAxisName, channel=ch, level=lev, MVAscore_relaxed_b10_cut=MVAscore_nom_relaxed_b10[cutMVA], syst=syst, weight=weights[cutMVA])  if len(MVAscore_nom_relaxed_b10)==len(weights) else print('')
...
#                   if lev in ['2j1b','3j1b','3j2b']:hout['costm_3'].fill(sample=histAxisName, channel=ch, level=lev, costm_3=cos2_abs[cut], syst=syst, weight=weights)
 ...
 #                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                      ...
                      if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                        ...
                        MVAscore_nom_relaxed_b10_fix = self.model_relaxed_b10_fix.predict_proba(np.where(np.isnan(variables),-99,variables))[:,1]
