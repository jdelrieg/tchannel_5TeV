...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        if isinstance(model, list) and len(model) >= 2:
          self.model_2j1b = model[0]
          self.model_3j2b = model[1]
          self.model_one = model[1]
...
#              sum_nones = sum(x for x in cut if x is  None)
 #             print('nones in:',ch,lev,sum_nones,'\n')
              #sum_nones_u0= sum(x for x in u0eta if x is  None)
  #            print('u0eta  in:',ch,lev,*u0eta,'\n')
              
              
#              if ch in ['e_fake','m_fake']:
 #               cuts      = [ch] + [lev] + ['metg30'] + ['METfilters']
  #              cutjets = GetCutJets(cuts, syst, met_pt, njets_var, nbtags_var)
   #             cut = (cutsel) & (cutjets) & (cutmtw) & (cutht)
    #            cut=ak.flatten(cut)
     #           cut = np.array(cut, dtype=bool)
        ...
        #        cutnomet = (cutsel) & (cutjets) & (cutht)
         #       cutnomet=ak.flatten(cutnomet)
          #      cutnomet = np.array(cutnomet, dtype=bool)  
              ...
              #flat_ht_atlas = ak.to_numpy(ak.flatten(ht_atlas))
