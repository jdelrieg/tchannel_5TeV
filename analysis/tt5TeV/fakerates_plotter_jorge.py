...
from analysis.tt5TeV.config_jorge import *
import time
...
def fakerates(chan, level):
  cat     = {'channel':chan, 'level':level}
  catfake = {'channel':chan[0] + '_fake', 'level':level}
  ...
  print('factll',factll,'data_metl20',data_metl20,'mc_metl20',mc_metl20,'data_metl20_fk',data_metl20_fk,'mc_metl20_fk',mc_metl20_fk)
  ...
  print('factlh',factlh,'data_metl20',data_metl20,'mc_metl20',mc_metl20,'data_metl20_fk',data_metl20_fk,'mc_metl20_fk',mc_metl20_fk)
  ...
  print('facthl',facthl,'data_metl20',data_metl20,'mc_metl20',mc_metl20,'data_metl20_fk',data_metl20_fk,'mc_metl20_fk',mc_metl20_fk)
  ...
  print('facthh',facthh,'data_metl20',data_metl20,'mc_metl20',mc_metl20,'data_metl20_fk',data_metl20_fk,'mc_metl20_fk',mc_metl20_fk)
...
'''
factll,factlh,facthl,facthh=fakerates_incl_total('e')
print('inclusivos para electrones',factll,factlh,facthl,facthh)
factll,factlh,facthl,facthh=fakerates_incl_total('m')
...
eta_values=np.array([-5, -0.8, 0.8, 5])
pt_values=np.array([0,90,200])
...
def plot_factors(channel, level):
    ...
    for i in range(len(eta_data["values"])-1):  
        plot.hlines(eta_data["factor_low_pt"][i], eta_data["values"][i], eta_data["values"][i+1], colors='blue', linewidth=2,label=r'p$_{\mathrm{T}}<90$ GeV' if i == 0 else "")
    for i in range(len(eta_data["values"])-1):  
        plot.hlines(eta_data["factor_high_pt"][i], eta_data["values"][i], eta_data["values"][i+1], colors='red', linewidth=2,label=r'p$_{\mathrm{T}}\geq90$ GeV' if i == 0 else "")
    ...
    plot.legend()
    plot.grid(True)
    plot.savefig(f"FakeRates_jorge/corrected_plots/FR_eta_{channel}_{level}.png")
    ...
    plot.legend()
    plot.grid(True)
    plot.savefig(f"FakeRates_jorge/corrected_plots/FR_pt_{channel}_{level}.png")
