...
for var in varlist:
  if not CheckHistoCategories(plt.GetHistogram(var), {'channel' : 'lep_pluss_fake', 'process':['data']+bkglist_noQCD}, checkValues=True) and not CheckHistoCategories(plt.GetHistogram(var), {'channel' : 'lep_minus_fake', 'process':['data']+bkglist_noQCD}, checkValues=True):
    print(', ', var, end='')
    continue
...
def GetQCDforVar(var):
  ...
  catfake = {'channel' : ['lep_pluss_fake', 'lep_minus_fake']}
  ...
  h_data_fake = GroupKeepOrder(h_data_fake, [['channel', 'channel', {'lep_pluss':'lep_pluss_fake', 'lep_minus':'lep_minus_fake'}], ['process', 'process', {'QCD':'data'       }]])
  h_mc_fake   = GroupKeepOrder(h_mc_fake  , [['channel', 'channel', {'lep_pluss':'lep_pluss_fake', 'lep_minus':'lep_minus_fake'}], ['process', 'process', {'QCD':bkglist_noQCD}]])
  h_mc_fake.scale(-1*lumi)
  FillDataSystCategories(h_data_fake)#, var=var)
  htot = (h_data_fake + h_mc_fake)
  hd  = h_data_fake.integrate('channel', 'lep_pluss').integrate('process').integrate('level', '2j1b').integrate('syst', 'norm') #4j2b
  hmc = h_mc_fake.integrate('channel', 'lep_pluss').integrate('process').integrate('level', '2j1b').integrate('syst', 'norm')   #4j2b
  ht  = htot.integrate('channel', 'lep_pluss').integrate('process').integrate('level', '2j1b').integrate('syst', 'norm')        #4j2b
  if htot._sumw != {}: 
...
def GetQCDnorm(chan, level, sys=0):
  ...
  if   sys== 1: countsData = 'counts_metl180' #l25 original #l180  ht
  elif sys==-1: countsData = 'counts_metl170' #l15			#l175
  else:         countsData = 'counts_metl175' #l20          #l170
