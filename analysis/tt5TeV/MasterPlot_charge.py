...
def CreateHistos(plt, systematics, process, channels, levels):
    ...
    mvascore = plt.GetHistogram('MVAscore_relaxed_b10')
    #mvascore = plt.GetHistogram('eeta')
    drjj     = plt.GetHistogram('counts')#medianDRjj
    iteration = 0
    ...
    for c in channels:
        for l in levels:
            for pr in process:
                ...
                if l == '1j1b':nbins=1; binshape= np.linspace(bin0, bin0 + nbins -1, nbins, dtype=np.float64)
...
def DrawMasterHistogram(fname):
    ...
    ax.text(0.63, 0.90, '$\mu^{+}+$jets', horizontalalignment='center', verticalalignment='center', transform=ax.transAxes, fontsize=15, color=colorchan)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(fname) or force:
        plt = plotter(path, prDic=processDic,  bkgList=bkglist, lumi=lumi, var=['counts', 'MVAscore_relaxed_b10', 'medianDRjj'])
        RebinVar(plt, 'MVAscore_relaxed_b10')
        ...
        #DrawShapesMaser(fname, 'e_plus', False)
