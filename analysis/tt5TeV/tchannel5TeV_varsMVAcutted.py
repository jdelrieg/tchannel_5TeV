...
#QCD ex    
  ...
  if 'g2jets' in cuts:
    ...
    jetcat = (njets == 3) & ((nbtags == 1) | (nbtags == 2))
  elif '4j1b' in cuts:
...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, model):
        ...
        'absu0eta'      : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("absu0eta", "$|\eta_{u_{0}}|$", 10, 0, 5)),        
...
########## hists i added end
        ...
        'absu0eta_cut'      : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("absu0eta_cut", "$|\eta_{u_{0}}|$", 10, 0, 5)),        
        'deltaeta_cut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("deltaeta_cut",      '|$\Delta\eta(\mathrm{u_{0},b_{0}})$|', 16, 0, 5.0)),
        'topmass_cut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("topmass_cut",      " $M_{top}$ ", 20, 100, 500)),        
        'mt_cut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("mt_cut",      "m$_{T}$ (GeV)", 10, 0, 150)),
        'medianDRjj_cut' : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("medianDRjj_cut", "median$\Delta$R(jj) ", 15, 0, 4.5)),
        'mjj_cut'        : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("mjj_cut",     "m(jj)( (GeV)", 24, 0, 480)),
        'sumallpt_cut'     : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("sumallpt_cut", "H$_{T}(\ell,\mathrm{j})$ (GeV)", 10, 0, 300)),        
        'mlb_cut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("mlb_cut",      "m$_{\ell b}$ (GeV)", 24, 0, 480)),        
        'mub_cut'        : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("mub_cut",     "m(u,b) (GeV)", 24, 0, 480)),
        'ht_atlas_cut'         : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("ht_atlas_cut",      "H'$_{T}$ (GeV)", 29, 5, 435)),
        'DptWub_cut'        : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("DptWub_cut",     "|$\Delta_{p_{T}}$(W,ub)| (GeV)", 10, 0, 200)),
        'DphiWub_cut'        : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("DphiWub_cut",     "|$\Delta\phi$(W,ub)| ", 10, 0, 5)),
        'Detalu_cut'        : hist.Hist("Events", hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("level", "level"), hist.Cat('syst', 'syst'), hist.Bin("Detalu_cut",     "|$\Delta\eta(\ell,u)$| ", 10, 0, 5)),
...
#        '2j1b_DRlu' : processor.column_accumulator(np.array([])),
    ...
    def process(self, events):
        ...
        if not isData and not isSystSample: systList = systList + ["elecSFUp","elecSFDown", "muonSFUp", "muonSFDown","btagSFlightUp","btagSFlightDown","btagSFbcUp","btagSFbcDown", "prefireUp", "prefireDown", "trigSFUp", "trigSFDown"]+systJets#, "btagSFUp", "btagSFDown",] + systJets
...
#              cutmet_mtw=met_pt+mtw>70  #For adding cut in sum of MET and Mw (me)
 #             cutmet_mtw=ak.fill_none(cutmet_mtw,False)
              ...
              cutht=ht_atlas>170#170  #For adding cut in Ht (me) Beware this is a different definition from our usual Ht
...
######################################  Redefine cuts from here if wanting to add something on the w-top system ##############################################################
              ...
              if histAxisName_control in ['W0JetsToLNu','W1JetsToLNu','W2JetsToLNu','W3JetsToLNu','WJetsToLNu']:
              ...
              jetcategories=['dummy']
              if histAxisName_control in ['W0JetsToLNu','W1JetsToLNu','W2JetsToLNu','W3JetsToLNu','WJetsToLNu']:jetcategories=['heavyjets','lightjets']
              for cat in jetcategories:
               ...
               if fillAcc and not isData and syst=='norm' and ch in ['e_plus', 'm_plus','e_minus','m_minus'] and lev in ['2j1b']:    
                hout[f'{lev}_njets'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_njets'].value.data, njets[cut].to_numpy()]))
                hout[f'{lev}_nbtags'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_nbtags'].value.data, nbtags[cut].to_numpy()]))
                hout[f'{lev}_ht'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_ht'].value.data, ht_var[cut].to_numpy()]))
                hout[f'{lev}_st'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_st'].value.data, st[cut].to_numpy()]))
                hout[f'{lev}_sumAllPt'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_sumAllPt'].value.data, ptSumVecAll[cut].to_numpy()]))
                hout[f'{lev}_leta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_leta'].value.data, ak.flatten(l_sel.eta[cut]).to_numpy()]))
                hout[f'{lev}_j0pt'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_j0pt'].value.data, jet0pt[cut].to_numpy()]))
                hout[f'{lev}_j0eta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_j0eta'].value.data, jet0eta[cut].to_numpy()]))
                hout[f'{lev}_u0pt'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_u0pt'].value.data, u0pt[cut].to_numpy()]))
                hout[f'{lev}_u0eta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_u0eta'].value.data, u0eta[cut].to_numpy()]))

                hout[f'{lev}_ptjj'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_ptjj'].value.data, ak.flatten(ptjj[cut]).to_numpy()]))
                hout[f'{lev}_mjj'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_mjj'].value.data, ak.flatten(mjj[cut]).to_numpy()]))
                hout[f'{lev}_medianDRjj'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_medianDRjj'].value.data, ak.flatten(drjjmedian[cut]).to_numpy()]))
                hout[f'{lev}_minDRjj'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_minDRjj'].value.data, ak.flatten(drjj[cut]).to_numpy()]))
                hout[f'{lev}_mlb'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_mlb'].value.data, mlb[cut].to_numpy()]))
                hout[f'{lev}_mt'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_mt'].value.data, ak.flatten(GetMT(l_sel, met)[cut]).to_numpy()]))
                hout[f'{lev}_ptsumveclb'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_ptsumveclb'].value.data, ptSumVeclb[cut].to_numpy()]))
                hout[f'{lev}_drlb'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_drlb'].value.data, ak.flatten(dRlb[cut]).to_numpy()]))

                hout[f'{lev}_absu0eta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_absu0eta'].value.data, abs(u0eta[cut]).to_numpy()]))
                hout[f'{lev}_ht_atlas'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_ht_atlas'].value.data, ht_atlas[cut].to_numpy()]))
                hout[f'{lev}_mtw'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_mtw'].value.data, mtw[cut]]))
                hout[f'{lev}_beta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_beta'].value.data, b0eta[cut].to_numpy()]))
                hout[f'{lev}_deltaeta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_deltaeta'].value.data, abs(b0eta - u0eta)[cut].to_numpy()]))
                hout[f'{lev}_topmass'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_topmass'].value.data, invariant_masses[cut].to_numpy()]))
                hout[f'{lev}_u0mass'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_u0mass'].value.data, u0mass[cut].to_numpy()]))
                hout[f'{lev}_absleta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_absleta'].value.data, abs(ak.flatten(lep_eta))[cut].to_numpy()]))
                hout[f'{lev}_topeta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_topeta'].value.data, top_eta[cut].to_numpy()]))
                hout[f'{lev}_absweta'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_absweta'].value.data, abs(w_etas)[cut]]))
                hout[f'{lev}_drub'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_drub'].value.data, dRub[cut].to_numpy()]))
                hout[f'{lev}_cost2'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_cost2'].value.data, cos2[cut].to_numpy()]))

                hout[f'{lev}_DptWub'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_DptWub'].value.data, DptWub[cut].to_numpy()]))
                hout[f'{lev}_mub'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_mub'].value.data, mub[cut].to_numpy()]))
                hout[f'{lev}_DphiWub'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_DphiWub'].value.data, DphiWub[cut].to_numpy()]))
                hout[f'{lev}_Detalu'] = processor.column_accumulator(np.concatenate([hout[f'{lev}_Detalu'].value.data, Detalu[cut].to_numpy()]))  
...
#                   if lev in ['2j1b','3j1b','3j2b']:hout['coste_3'].fill(sample=histAxisName, channel=ch, level=lev, coste_3=cos2_abs[cut], syst=syst, weight=weights)
 ...
 #                     hout['mlb'].fill(sample=histAxisName, channel=ch, level=lev, mlb=mlb, syst=syst, weight=weights)
                      if fillMVA and lev in ['2j1b','3j1b','3j2b'] and self.model_2j1b is not None and len(fht) > 0:
                        ...
                        MVAscore_nom_relaxed_b10_all = self.model_relaxed_b10_all.predict_proba(np.column_stack(varsb10))[:,1]
                        MVAscore_nom_relaxed_b10_anal = self.model_relaxed_b10_anal.predict_proba(np.column_stack(varsb10))[:,1]
                        MVAscore_nom_relaxed_b10_25 = self.model_relaxed_b10_25.predict_proba(np.column_stack(varsb10))[:,1]
                        MVAscore_nom_relaxed_b10_inter = self.model_relaxed_b10_inter.predict_proba(np.column_stack(varsb10))[:,1]
                        
                        MVAscore_nom_nomt=self.model_no_mt.predict_proba(np.column_stack(vars_nomt))[:,1]
                        ...
                        hout['absu0eta_cut'].fill(sample=histAxisName, channel=ch, level=lev, absu0eta_cut=abs(u0eta)[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['deltaeta_cut'].fill(sample=histAxisName, channel=ch, level=lev, deltaeta_cut=abs(b0eta-u0eta)[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['topmass_cut'].fill(sample=histAxisName, channel=ch, level=lev, topmass_cut=invariant_masses[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['mt_cut'].fill(sample=histAxisName, channel=ch, level=lev, mt_cut=mt[cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['medianDRjj_cut'].fill(sample=histAxisName, channel=ch, level=lev, medianDRjj_cut=fdrjjmed[cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['mlb_cut'].fill(sample=histAxisName, channel=ch, level=lev, mlb_cut=mlb[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['mjj_cut'].fill(sample=histAxisName, channel=ch, level=lev, mjj_cut=fmjj[cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['mub_cut'].fill(sample=histAxisName, channel=ch, level=lev, mub_cut=mub[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['sumallpt_cut'].fill(sample=histAxisName, channel=ch, level=lev, sumallpt_cut=fptSumVecAll[cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['ht_atlas_cut'].fill(sample=histAxisName, channel=ch, level=lev, ht_atlas_cut=ht_atlas[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['DptWub_cut'].fill(sample=histAxisName, channel=ch, level=lev, DptWub_cut=DptWub[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['DphiWub_cut'].fill(sample=histAxisName, channel=ch, level=lev, DphiWub_cut=DphiWub[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
                        hout['Detalu_cut'].fill(sample=histAxisName, channel=ch, level=lev, Detalu_cut=Detalu[cut][cutMVA], syst=syst, weight=weights[cutMVA])  
