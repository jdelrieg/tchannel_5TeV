...
# Iterate over all combinations of channels and levels
for channel in channels:
    for level in levels:
        # Compute the fake rate using GetQCDnorm
        fake_rate = GetQCDnorm(channel, level)
        fake_rates[(channel, level)] = fake_rate

        # Compute the CR rate using GetQCDnorm_v2
        cr_rate = GetQCDnorm_v2(channel, level)
        cr_rates[(channel, level)] = cr_rate

if (joint==True) & ('3jg1b' in levels):
  print('We are joining the 3j1b and 3j2b categories')
  for channel in ['e_plus','e_minus', 'm_plus']:
    for level in ['3j1b','3j2b']:
      fake_rates[(channel, level)]=fake_rates[(channel, '3jg1b')]   #3jg1b
  fake_rates[('m_minus','3j1b')]=fake_rates[('m_minus','3j2b')]

# Convert tuple keys to strings for JSON compatibility
fake_rates_str_keys = {str(k): v for k, v in fake_rates.items()}
cr_rates_str_keys = {str(k): v for k, v in cr_rates.items()}

# Save the rates to a JSON file
output_rates = {
    "fake_rates": fake_rates_str_keys,
    "cr_rates": cr_rates_str_keys
}

with open(path+"rates_QCD.json", "w") as f:
    json.dump(output_rates, f)

print("Fake rates and CR rates saved to %srates_QCD.json"%(path))
