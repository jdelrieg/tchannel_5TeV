import os, sys, argparse, uproot, json
import subprocess
...
def GetSumWeightsFromRunsTree(fname):
  fname0 = fname if not isinstance(fname, list) else fname[0]
  f = uproot.open(fname0)
  t = f['Runs']
  # Scales
  nscaleweights = t['nLHEScaleSumw'].array()[0]
  sumscales = t['LHEScaleSumw'].array()
  sumscales = np.sum(sumscales, axis=0)
  sumscales = sumscales.to_numpy()
  # PDFs
  npdfweights   = t['nLHEPdfSumw'].array()[0]
  sumpdfs  = t['LHEPdfSumw'].array()
  sumpdfs  = np.sum(sumpdfs, axis=0)
  sumpdfs  = sumpdfs.to_numpy()
  # sumwpdf = np.zeros(npdfweights); sumwscales = np.zeros(nscaleweights);
  f.close()
  if not isinstance(fname, list): fname = [fname]
  for fi in fname[1:]:
    f = uproot.open(fi)
    t = f['Runs']
    scales = t['LHEScaleSumw'].array()
    scales = np.sum(scales, axis=0)
    scales = scales.to_numpy()
    sumscales = sumscales+scales
    pdfs  = t['LHEPdfSumw'].array()
    pdfs  = np.sum(pdfs, axis=0)
    pdfs  = pdfs.to_numpy()
    sumpdfs += pdfs
    f.close()
  return sumpdfs, sumscales


def GetFilesFromJson(jsonFiles):
  if   isinstance(jsonFiles, str) and ',' in jsonFiles: jsonFiles = jsonFiles.replace(' ', '').split(',')
  if isinstance(jsonFiles, list):
    files = [] 
    for j in jsonFiles:
      files += GetFilesFromJson(j)
      return files
  ## Load jston and extract files
  with open(jsonFiles) as jf:
    dic = json.load(jf)
    return dic['files']


'''
root [3] Runs->Scan("nLHEScaleSumw")
************************
*    Row   * nLHEScale *
************************
*        0 *         9 *
************************
(Long64_t) 1
root [4] Runs->Scan("LHEScaleSumw")
***********************************
*    Row   * Instance * LHEScaleS *
***********************************
*        0 *        0 * 1.1259392 *
*        0 *        1 * 1.1068764 *
*        0 *        2 * 1.0946667 *
